/*
  ...
  1. Changes
    - Add unique constraint on room names
...
-- Get Williams College school_id and insert rooms
DO $$ 
DECLARE
  williams_id uuid;
BEGIN
  -- Get Williams College ID
  SELECT id INTO williams_id 
  FROM schools 
  WHERE code = 'WILLIAMS';

  -- Update existing rooms to be associated with Williams College
  UPDATE rooms 
  SET school_id = williams_id 
